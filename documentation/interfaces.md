# 🟫 Consumer Interfaces — System Design Deep Dive

---

## 🎯 Zone Responsibility

Interfaces has one sacred job:
...
If Serving is the **reasoning brain**, Interfaces is the **sensory-motor interface** that actually touches the developer. 
...
Four core principles govern the design of Interfaces:
...
The following map illustrates how Interfaces interfaces coordinate with the Serving Layer (Serving) and AI clients:
...
The server exposes the following tools to the AI client. When the LLM calls these tools, they execute corresponding operations on Serving's serving layer.
...
- **`archgraph://drift/log`**: Returns a log of recent architectural changes appended to the Delta Log in Storage.
...
```
Editor Action (Focus/Edit) ──▶ [WebSocket Client] ──▶ Serving API ──▶ Real-time Diagnostics
```

### 1. Sidebar Architecture Assistant
An in-editor chat panel integrated with the local IDE's active document context.
- **Context Injection:** When the developer asks, *"How does this file fit into the system?"*, the plugin automatically extracts the active file name, maps it to its canonical entity ID via the IDE's local file mapping cache, and queries Serving's Query Engine.
...
Maps architectural smells returned from Serving directly onto the IDE's editor window as wiggly warning lines.
...
An interactive UI slider that steps through historical snapshots from Storage.
...
```
Git Push ──▶ CI Trigger ──▶ [PR Analyzer Runner] ──▶ [Serving API] ──▶ PR Comment / Check Gate
```
...
3. Requests a blast radius report for those modified entities from Serving.
...
| **Tool Execution** | MCP Agent Call | **< 150ms** | Pre-compute common path traversals in Serving |
