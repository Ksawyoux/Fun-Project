...
Reaches into signal sources (Signal Sources), normalizes records into NIF
(Normalized Ingestion Format), and delivers them to Pipeline by default
(or to Storage directly for debug/dev, or to a file for testing). Source isolation: nothing downstream knows about git,
...
- Author/Person entities (requires Storage type extension).

## Run

Requires `pipelined` listening for the default path (see top-level `cmd/archgraph`).
...
For direct debug/dev writes to Storage, pass `-zone4 http://localhost:8080`.
