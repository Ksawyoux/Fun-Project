...
// API models mirroring Storage and Serving schemas
type AskReq struct {
...
func main() {
	// Base configuration
	storageAddr := flag.String("storage", "http://localhost:8080", "Storage Base URL")
	servingAddr := flag.String("serving", "http://localhost:8081", "Serving Base URL")
	...
	switch subcommand {
	case "query":
		handleQuery(ctx, *servingAddr, cfg.Namespace, subArgs)
	case "diff":
		handleDiff(ctx, *servingAddr, cfg.Namespace, subArgs)
	case "impact":
		handleImpact(ctx, *storageAddr, *servingAddr, cfg.Namespace, subArgs)
	case "validate":
		handleValidate(ctx, *storageAddr, *servingAddr, cfg, subArgs)
	case "graph":
		handleGraph(ctx, *storageAddr, cfg.Namespace, subArgs)
	case "document":
		handleDocument(ctx, *storageAddr, *servingAddr, cfg, subArgs)
	case "mcp":
...
func printUsage() {
	...
	fmt.Println("\nFlags:")
	fmt.Println("  -storage  string  Storage base URL (default: http://localhost:8080)")
	fmt.Println("  -serving  string  Serving base URL (default: http://localhost:8081)")
...
func handleQuery(ctx context.Context, servingAddr, namespace string, args []string) {
	if len(args) < 1 {
	...
	resp, err := postJSON(ctx, servingAddr+"/v1/ask", reqBody)
	if err != nil {
		fmt.Printf("Error contacting Serving: %v\n", err)
		os.Exit(1)
	}
...
func handleDiff(ctx context.Context, servingAddr, namespace string, args []string) {
	if len(args) < 2 {
	...
	resp, err := postJSON(ctx, servingAddr+"/v1/diff", reqBody)
	if err != nil {
		fmt.Printf("Error contacting Serving: %v\n", err)
		os.Exit(1)
	}
...
func handleImpact(ctx context.Context, storageAddr, servingAddr, namespace string, args []string) {
	var fileArg string
	var lineArg int
	var maxDepth int
	...
	if fileArg != "" {
		resolvedID, err := findEntityByFileAndLine(ctx, storageAddr, namespace, fileArg, lineArg)
		if err != nil {
	...
	resp, err := postJSON(ctx, servingAddr+"/v1/blast-radius", reqBody)
	if err != nil {
		fmt.Printf("Error contacting Serving: %v\n", err)
		os.Exit(1)
	}
...
func handleValidate(ctx context.Context, storageAddr, servingAddr string, cfg *Config, args []string) {
	var detail bool
	fs := flag.NewFlagSet("validate", flag.ExitOnError)
	fs.BoolVar(&detail, "detail", false, "Show detailed rule breakdown")
	_ = fs.Parse(args)

	// 1. Fetch Smells from Serving Health Audit
	reqBody, _ := json.Marshal(map[string]string{
		"namespace": cfg.Namespace,
	})

	resp, err := postJSON(ctx, servingAddr+"/v1/health-audit", reqBody)
	if err != nil {
		fmt.Printf("Error contacting Serving audit: %v\n", err)
		os.Exit(1)
	}
	defer resp.Body.Close()
	...
	allEntities, err := fetchAllEntities(ctx, storageAddr, cfg.Namespace)
	if err != nil {
...
func fetchAllEntities(ctx context.Context, storageAddr, namespace string) ([]*Entity, error) {
	u := fmt.Sprintf("%s/v1/entities?namespace=%s", storageAddr, url.QueryEscape(namespace))
	resp, err := http.Get(u)
	if err != nil {
		return nil, err
	}
...
func findEntityByFileAndLine(ctx context.Context, storageAddr, namespace string, path string, line int) (string, error) {
	entities, err := fetchAllEntities(ctx, storageAddr, namespace)
	if err != nil {
		return "", err
	}
...
func handleGraph(ctx context.Context, storageAddr, namespace string, args []string) {
	var format string
	fs := flag.NewFlagSet("graph", flag.ExitOnError)
	fs.StringVar(&format, "format", "tree", "Output format: tree | mermaid")
	_ = fs.Parse(args)

	listing, err := fetchNamespaceListing(ctx, storageAddr, namespace)
	if err != nil {
...
func fetchNamespaceListing(ctx context.Context, storageAddr, namespace string) (*NamespaceListing, error) {
	u := fmt.Sprintf("%s/v1/entities?namespace=%s", storageAddr, url.QueryEscape(namespace))
	resp, err := http.Get(u)
	if err != nil {
		return nil, err
...
func handleMCP(ctx context.Context, storageAddr, servingAddr, defaultNamespace string) {
	...
	for scanner.Scan() {
		...
		go handleRequest(ctx, storageAddr, servingAddr, defaultNamespace, req)
	}
}

func handleRequest(ctx context.Context, storageAddr, servingAddr, defaultNamespace string, req JSONRPCRequest) {
	switch req.Method {
	case "initialize":
	...
	case "tools/call":
		...
		res, err := callTool(ctx, storageAddr, servingAddr, defaultNamespace, params.Name, params.Arguments)
		if err != nil {
	...
	case "resources/read":
		...
		res, err := readResource(ctx, storageAddr, servingAddr, defaultNamespace, params.URI)
		if err != nil {
...
func callTool(ctx context.Context, storageAddr, servingAddr, defaultNamespace string, name string, args json.RawMessage) (*ToolCallResult, error) {
	switch name {
	...
	case "archgraph_suggestions":
		...
		entities, err := fetchAllEntities(ctx, storageAddr, ns)
		if err == nil {
			for _, e := range entities {
				if e.Type == "SERVICE" {
	...
	case "archgraph_blast_radius":
		...
		entityID := m.EntityID
		if entityID == "" && m.File != "" {
			resolved, err := findEntityByFileAndLine(ctx, storageAddr, ns, m.File, m.Line)
			if err != nil {
				return nil, err
			}
			entityID = resolved
	...
	case "archgraph_document":
		...
		docText, err := generateDocumentation(ctx, storageAddr, servingAddr, ns)
		if err != nil {
			return nil, err
		}
...
func readResource(ctx context.Context, storageAddr, servingAddr, defaultNamespace string, uri string) (*ResourceReadResult, error) {
	switch uri {
...
Relationships:
  ...
  DEPENDS_ON            - Structural dependencies.`
	...
	case "archgraph://drift/log":
		resp, err := http.Get(storageAddr + "/v1/log?limit=20")
		if err != nil {
			return nil, err
		}
		defer resp.Body.Close()
...
func handleDocument(ctx context.Context, storageAddr, servingAddr string, cfg *Config, args []string) {
	var outPath string
	fs := flag.NewFlagSet("document", flag.ExitOnError)
	fs.StringVar(&outPath, "out", "", "Path to write the markdown documentation")
	_ = fs.Parse(args)

	doc, err := generateDocumentation(ctx, storageAddr, servingAddr, cfg.Namespace)
	if err != nil {
...
func getExecutiveSummary(ctx context.Context, servingAddr, namespace string, entities []*Entity, relationships []*Relationship) string {
	reqBody, _ := json.Marshal(AskReq{
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, servingAddr+"/v1/ask", bytes.NewReader(reqBody))
	if err == nil {
		req.Header.Set("Content-Type", "application/json")
...
func generateDocumentation(ctx context.Context, storageAddr, servingAddr, namespace string) (string, error) {
	listing, err := fetchNamespaceListing(ctx, storageAddr, namespace)
	if err != nil {
	...
	execSummary := getExecutiveSummary(ctx, servingAddr, namespace, listing.Entities, listing.Relationships)
...
func handleAnalyzePR(ctx context.Context, storageAddr, servingAddr string, cfg *Config, args []string) {
	...
	for _, file := range modifiedFiles {
		resolvedID, err := findEntityByFileAndLine(ctx, storageAddr, cfg.Namespace, file, 0)
		if err != nil {
			// File might not map to a graph entity, skip it
			continue
		}
		// Fetch entity details from Serving
		u := fmt.Sprintf("%s/v1/entities/%s", servingAddr, url.PathEscape(resolvedID))
		resp, err := http.Get(u)
		if err != nil {
			continue
		}
		...
		if json.NewDecoder(resp.Body).Decode(&ent) == nil {
			...
			respBR, err := postJSON(ctx, servingAddr+"/v1/blast-radius", reqBody)
			if err == nil {
				var br BlastRadius
