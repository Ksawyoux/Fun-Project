...
import (
	...
	"archgraph/nif"
	"archgraph/pipeline/internal/registry"
...
func TestPipelineProcessingFlow(t *testing.T) {
	...
	var receivedMutations []storageclient.Mutation
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")

		if r.Method == http.MethodGet && r.URL.Path == "/v1/entities" {
			// Mock ListNamespace
			resp := storageclient.NamespaceListing{
				Namespace:     r.URL.Query().Get("namespace"),
				Entities:      []*storageclient.Z4Entity{},
				Relationships: []*storageclient.Z4Relationship{},
			}
		...
		if r.Method == http.MethodPost && r.URL.Path == "/v1/mutations" {
			// Mock ApplyBatch
			var req struct {
				Mutations []storageclient.Mutation `json:"mutations"`
			}
	...
	for _, m := range mutations {
		if m.Kind == storageclient.KindUpsertEntity && m.Entity != nil {
		...
		if m.Kind == storageclient.KindUpsertRelationship && m.Relationship != nil {
