...
described in `Zone5.md`. Sits on top of Storage and consumes its REST API.
Defers anything that needs infra Storage doesn't yet have: snapshots,
...
| 2 | HTTP client to Storage (no shared library) | Storage's data types live under `internal/`. Going over HTTP avoids the temptation to leak schema and matches the production deployment shape. |
...
| 6 | One Neighborhood call per BFS step in Impact | Storage has no batch-traverse endpoint. Acceptable for MVP; production pushes the BFS into Storage. |
...
| 9 | Add `ListByNamespace` to Storage | Health Auditor needs a whole-workspace view; without it, Tarjan can't run. |
...
```
serving/
├── cmd/servingd/main.go                 HTTP entry point
├── internal/
...
| GET | `/v1/entities/{id}` | — | Passthrough to Storage (+ optional `?depth=N`) |
...
End-to-end tests against a live `storaged` are deferred until Go is installed
...
2. **No retries / circuit breaker** between Serving and Storage — a single
