...
func CalculateBlastRadius(ctx context.Context, cl *storageclient.Client, originID string, maxDepth int) (*BlastRadius, error) {
	if maxDepth < 1 {
		maxDepth = 5
	}
	if maxDepth > 5 {
	...
	for len(queue) > 0 {
		...
		nb, err := cl.Neighborhood(ctx, f.nodeID, 1, storageclient.DirIn)
		if err != nil {
