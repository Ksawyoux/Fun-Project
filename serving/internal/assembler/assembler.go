...
func Assemble(sub *retriever.Subgraph) Output {
	...
	var inbound, outbound []*storageclient.Relationship
...
func writeEntitySection(b *strings.Builder, header string, e *storageclient.Entity, score float64) {
	fmt.Fprintf(b, "### %s\n", header)
	fmt.Fprintf(b, "- **ID:** `%s`\n", e.ID)
...
func writeEdgeList(b *strings.Builder, edges []*storageclient.Relationship, nodeByID map[string]*retriever.ScoredNode, peerField string) {
...
func refOf(e *storageclient.Entity) CitationRef {
	return CitationRef{
		ID:            e.ID,
