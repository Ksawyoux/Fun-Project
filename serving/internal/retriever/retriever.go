...
type Subgraph struct {
	Origin *storageclient.Entity      `json:"origin"`
	Nodes  []*ScoredNode            `json:"nodes"`
	Edges  []*storageclient.Relationship `json:"edges"`
	// Stats for observability / debugging.
	Stats RetrieveStats `json:"stats"`
}

type ScoredNode struct {
	*storageclient.Entity
...
func Retrieve(ctx context.Context, cl *storageclient.Client, originID string, depth int, dir storageclient.Direction) (*Subgraph, error) {
	if depth < 1 {
		depth = 1
	}
	if depth > 5 {
...
func scoreAndPrune(nb *storageclient.Neighborhood) *Subgraph {
	...
	var keptEdges []*storageclient.Relationship
	for _, e := range nb.Edges {
