...
A single-process, SQLite-backed implementation of Storage (Graph Storage) per
...
| 8 | Deterministic IDs from `hash(type + canonical_name + namespace)` | Spec recipe minus `source_*` (no Ingestion yet). Two reports of the same entity collide to the same ID — intended. |
...
```
storage/
├── cmd/storaged/main.go              HTTP server entry point
├── internal/
